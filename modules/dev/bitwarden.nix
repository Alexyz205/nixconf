...
let
  # Options shared by the NixOS and home-manager sides. The NixOS side
  # additionally wires the CA into the system trust store.
  bitwardenOptions = {
    ...
    caCertFile = lib.mkOption {
      ...
      description = ''
        ...
        homelab cert-manager root CA. Added to the system trust store (fixes
        the desktop app + Chromium-based browsers) and exported as
        NODE_EXTRA_CA_CERTS (fixes the Node.js CLI). Null when the server uses
