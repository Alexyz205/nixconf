{
  ...
  tvCfg = {
    ...
    home.file.".config/television".source = config.lib.file.mkOutOfStoreSymlink "${config.home.homeDirectory}/repos/personal/nixconf/config/television";
  };
in {
  ...
  flake.modules.homeManager.tv = {
    ...
    options.modules.tv.enable = lib.mkEnableOption "Television (tv) fuzzy finder";
    config = lib.mkIf config.modules.tv.enable (tvCfg {inherit config pkgs;});
  };
}