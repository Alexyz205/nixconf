{ lib, ... }:
let
  commonAliases = import ./config/shell/aliases.nix;
  ...
  certsDir = toString ./config/certs;
  nixCaBundle = pkgs: pkgs.runCommand "nix-corporate-ca-bundle.crt" { } ''
    cat ${pkgs.cacert}/etc/ssl/certs/ca-bundle.crt \
      ${certsDir}/t2_ca_chain.crt \
      ${certsDir}/T2-Palo-root.crt > $out
  '';
in {
  flake.modules.nixos.shell = { config, lib, pkgs, ... }: {
    ...
    config = lib.mkIf config.modules.shell.enable {
      ...
      home-manager.users."alexis.pigeon" = {
