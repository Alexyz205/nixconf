    ...
    files = apps:
      builtins.listToAttrs (
        map (name: {
          ...
          value = {source = "${mkHide name}/${name}.desktop";};
        }) apps
      );
  in {
    ...
    config = lib.mkIf config.modules.hiddenApps.enable {
      home-manager.users.${config.modules.users.userName}.home.file = files config.modules.hiddenApps.apps;
    };
  };
}
